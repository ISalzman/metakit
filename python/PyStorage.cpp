...
static PyObject* PyStorage_Aside(PyStorage *o, PyObject* _args) {
	...
	if (!PyStorage_Check((PyObject*) args[0]))
	    throw PWException(PyExc_TypeError, "First arg must be a storage");
	c4_Storage& storage = *(PyStorage*)(PyObject*) args[0];
        if (!o->SetAside(storage))
             throw PWException(PyExc_IOError, "aside failed");
        Py_INCREF(Py_None);
        return Py_None;
    }
    catch(PWException e) {
...
static PyObject* PyStorage_new(PyObject* o, PyObject* _args) {
    try {
        ...
        switch (args.len()) {
            case 0:
                return new PyStorage;
            case 1:
                if (!PyFile_Check((PyObject* )args[0])) {
                    break;
                }
                return new PyStorage(*new c4_FileStrategy (PyFile_AsFile(args[0])), true);
            case 4: // Rrrrrr...
    {
        if (!PyStorage_Check((PyObject *)args[0]))
        ...
        return new PyStorage(*new SiasStrategy (storage, view, prop, row), true);
            ...
            case 2: {
                    ...
                    if (PyInt_Check((PyObject* )args[1])) {
                        ...
                        PyStorage* ps = new PyStorage(fnm, mode);
                        if (!ps->Strategy().IsValid())
                        {
                            delete ps;
                            throw PWException(PyExc_IOError,
                                "can't open storage file");
                        }
                        return ps;
                    }
            }
...
void initMk4py() {
    ...
    PyObject_SetAttrString(m, "version", PyString_FromString("2.3.4"));
}
#endif
