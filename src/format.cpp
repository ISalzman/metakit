...
void c4_FormatB::Define(int rows_, const t4_byte** ptr_)
{
    ...
    c4_ColOfInts memos (_data.Persist());

    if (ptr_ != 0)
    {
        _data.PullLocation(*ptr_);
        if (_data.ColSize() > 0)
            sizes.PullLocation(*ptr_);
        memos.PullLocation(*ptr_);
    }
    ...
    {
        c4_Bytes walk;
        memos.FetchBytes(0, memos.ColSize(), walk, true);
...
void c4_FormatB::Commit(c4_SaveContext& ar_)
{
    ...
    c4_Column memos (0);
    c4_Column* saved = ar_.SetWalkBuffer(&memos);
    ...
    {
        sizes.FixSize();
        ar_.CommitColumn(sizes);
    }
    
    ar_.CommitColumn(memos);
}
