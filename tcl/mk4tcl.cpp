...
int MkTcl::LoopCmd()
{
    ...
    Tcl_ObjSetVar2(interp, objv [1], 0, var,
                        TCL_LEAVE_ERR_MSG | TCL_PARSE_PART1);
...
int MkTcl::CursorCmd()
{
    ...
    {   
        var = Tcl_ObjGetVar2(interp, name, 0,
                                    TCL_LEAVE_ERR_MSG | TCL_PARSE_PART1);
        if (var == 0)
            return Fail(); // has to exist, can't be valid otherwise
    }
    ...
    Tcl_Obj* result = Tcl_ObjSetVar2(interp, name, 0, var,
                                    TCL_LEAVE_ERR_MSG | TCL_PARSE_PART1);
    if (result == 0)
        return Fail();
...
int MkTcl::Execute(int oc, Tcl_Obj* const* ov)
{
    ...
    // the "&MkTcl::" stuff is required for Mac cwpro2
        ...
        { &MkTcl::LoopCmd,      3,  7,  "loop cursorname ?path first limit incr? {command}" },
